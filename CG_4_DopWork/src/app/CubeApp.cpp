...
void CubeApp::Update(const FrameTimer& gt)
{
	...
	if (mKeyD) moveDir = XMVectorAdd(moveDir, right);    // Вправо
	if (mKeyA) moveDir = XMVectorSubtract(moveDir, right);  // Влево
	...
	// Матрица второго куба (прозрачный белый, больше красного и содержит его внутри)
	// Масштабируем белый куб в 2 раза и центрируем на красном кубе
	XMMATRIX scale = XMMatrixScaling(2.0f, 2.0f, 2.0f); // Увеличиваем в 2 раза
	XMMATRIX translation = XMMatrixTranslation(0.0f, 0.0f, 0.0f); // Центрируем на красном кубе
	XMMATRIX secondBoxWorld = scale * translation;
	XMStoreFloat4x4(&mSecondBoxWorld, secondBoxWorld);
	
	XMMATRIX secondBoxWvp = secondBoxWorld * view * proj;
	
	ObjectConstants secondObj{};
	XMStoreFloat4x4(&secondObj.World, XMMatrixTranspose(secondBoxWorld));
	XMStoreFloat4x4(&secondObj.WorldInvTranspose, XMMatrixTranspose(MathUtils::InverseTranspose(secondBoxWorld)));
	XMStoreFloat4x4(&secondObj.WorldViewProj, XMMatrixTranspose(secondBoxWvp));
	secondObj.EyePosW = mCameraPos;
	secondObj.LightDirW   = XMFLOAT3(0.577f, -0.577f, 0.577f);
	secondObj.LightColor  = XMFLOAT3(1.0f, 1.0f, 1.0f);
	secondObj.AmbientK    = 0.15f;
	secondObj.SpecPower   = 64.0f;
	
	// Обновляем константный буфер для второго куба (индекс 1)
	mObjectCB->CopyData(1, secondObj);
}
...
void CubeApp::Draw(const FrameTimer& gt)
{
    ...
    // Рисуем сначала белый куб (прозрачный, большой, снаружи)
    // Переключаемся на прозрачный PSO
    mCommandList->SetPipelineState(mTransparentPSO.Get());
    
    // Устанавливаем буферы вершин и индексов для белого куба
    const auto vbv2 = mSecondBoxGeo->VertexBufferView();
    mCommandList->IASetVertexBuffers(0, 1, &vbv2);
    const auto ibv2 = mSecondBoxGeo->IndexBufferView();
    mCommandList->IASetIndexBuffer(&ibv2);
    
    // Устанавливаем дескриптор для белого куба (индекс 1)
    CD3DX12_GPU_DESCRIPTOR_HANDLE cbvHandle(mCbvHeap->GetGPUDescriptorHandleForHeapStart(), 1, mCbvSrvUavDescriptorSize);
    mCommandList->SetGraphicsRootDescriptorTable(0, cbvHandle);
    
    mCommandList->DrawIndexedInstanced(
        mSecondBoxGeo->DrawArgs["box"].IndexCount,
        1, 0, 0, 0);
    
    // Рисуем красный куб (внутри белого)
    // Переключаемся обратно на непрозрачный PSO
    mCommandList->SetPipelineState(mPSO.Get());
    
    // Устанавливаем буферы вершин и индексов для красного куба
    mCommandList->IASetVertexBuffers(0, 1, &vbv);
    mCommandList->IASetIndexBuffer(&ibv);
...
{
    (void)btnState; // Неиспользуемый параметр
    mLastMousePos.x = x;
    mLastMousePos.y = y;
...
{
    (void)btnState; // Неиспользуемый параметр
    (void)x; // Неиспользуемый параметр
    (void)y; // Неиспользуемый параметр
    ReleaseCapture();
}
...
void CubeApp::BuildDescriptorHeaps()
{
    ...
    cbvHeapDesc.NumDescriptors = 2; // 2 объекта
...
{
	mObjectCB = std::make_unique<GpuUploadBuffer<ObjectConstants>>(md3dDevice.Get(), 2, true); // 2 объекта
	...
	// Второй CBV для второго куба
	cbAddress += objCBByteSize; // Следующий объект
	cbvDesc.BufferLocation = cbAddress;
	
	// Получаем дескриптор для второго куба (смещение на 1)
	CD3DX12_CPU_DESCRIPTOR_HANDLE secondCbvHandle(mCbvHeap->GetCPUDescriptorHandleForHeapStart());
	secondCbvHandle.Offset(1, mCbvSrvUavDescriptorSize);
...
void CubeApp::BuildSecondBoxGeometry()
{
    const XMFLOAT4 cubeColor = XMFLOAT4(1.0f, 1.0f, 1.0f, 0.3f); // прозрачный белый (альфа 0.3 - более прозрачный)
    ...
    mSecondBoxGeo = std::make_unique<MeshGeometry>();
    mSecondBoxGeo->Name = "SecondBox";

    ThrowIfFailed(D3DCreateBlob(vbByteSize, &mSecondBoxGeo->VertexBufferCPU));
    CopyMemory(mSecondBoxGeo->VertexBufferCPU->GetBufferPointer(), vertices.data(), vbByteSize);

    ThrowIfFailed(D3DCreateBlob(ibByteSize, &mSecondBoxGeo->IndexBufferCPU));
    CopyMemory(mSecondBoxGeo->IndexBufferCPU->GetBufferPointer(), indices.data(), ibByteSize);

    mSecondBoxGeo->VertexBufferGPU = Dx12Utils::CreateDefaultBuffer(
        md3dDevice.Get(), mCommandList.Get(),
        vertices.data(), vbByteSize,
        mSecondBoxGeo->VertexBufferUploader
    );

    mSecondBoxGeo->IndexBufferGPU = Dx12Utils::CreateDefaultBuffer(
        md3dDevice.Get(), mCommandList.Get(),
        indices.data(), ibByteSize,
        mSecondBoxGeo->IndexBufferUploader
    );

    mSecondBoxGeo->VertexByteStride = sizeof(Vertex);
    mSecondBoxGeo->VertexBufferByteSize = vbByteSize;
    mSecondBoxGeo->IndexFormat = DXGI_FORMAT_R16_UINT;
    mSecondBoxGeo->IndexBufferByteSize = ibByteSize;
    ...
    mSecondBoxGeo->DrawArgs["box"] = submesh;
}
...
void CubeApp::BuildPSO()
{
    ...
    // Создаем PSO для прозрачного куба
    D3D12_GRAPHICS_PIPELINE_STATE_DESC transparentPsoDesc = psoDesc;
    
    // Настраиваем blend state для прозрачности
    ...
    // Отключаем depth write для прозрачных объектов (чтобы они правильно смешивались)
